...
public class LoginFilter implements Filter  {
    ...
    public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain) throws IOException, ServletException {
         try {
            ...
            { 
               System.out.println("La sesion no es nula , tam de la lista:"+((List<listaMenu>)ses.getAttribute("listaUsuarioRol")).size());
            }   
            else{
            ...
            else{ if (comparaLista((List<listaMenu>)ses.getAttribute("listaUsuarioRol"),reqPath) || reqPath.endsWith("/modulo.xhtml")) {
                       chain.doFilter(request, response);
                      } 
                  else{
                   res.sendRedirect(req.getContextPath() + "/home.xhtml");  // Anonymous user. Redirect to login page*/
                  }
